...
if ( ! class_exists( 'ACF_Admin_UI_Options_Page' ) ) :
	...
	class ACF_Admin_UI_Options_Page extends ACF_Admin_Internal_Post_Type {
		...
		/**
		 * Iterates through the registered options page and finds eligible parent page.
		 *
		 * @since 6.2
		 *
		...
		public static function get_parent_page_choices( $menu_slug = '' ) {
			...
			if ( is_array( $all_options_pages ) ) {
				foreach ( $all_options_pages as $options_page ) {
					...
					// ACF overrides the `menu_slug` of parent page with one child so they redirect to the child.
